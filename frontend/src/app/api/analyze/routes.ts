...
const MODEL = "llama-3.3-70b-versatile"; // swap to openai/gpt-oss-120b if you want stronger reasoning
...
// Fake step choreography — gives the UI the "7 agents" feel from one real call
const STEP_NAMES = [
  "STT preprocessor",
  "Lexical agent",
  "Semantic agent",
...
export async function POST(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // 1. Kick off the real Groq call immediately (don't block on fake steps)
      ...
      // 2. Play the fake step sequence while the real call runs in parallel
      ...
      // 3. Resolve the real call, parse, emit end
      try {
